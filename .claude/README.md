...
focused on the deliverable Voxfit will review; this branch ships the full AI-assistant
setup used during development.

## Contents

- **`dispatcher.md`** — Concierge / routing logic for the AI-assistant orchestrator.
  When a question lands on this repo, match it to a category (project intro /
  operational / code-lookup / rationale / edge-case / coverage / feature / audit) and
  dispatch to the right subagent. Includes a self-introduction template and 15 worked
  example questions from simple to most pointed.
...
- **`../CLAUDE.md`** (repo root, shipped on `main` too) — hard rules for any AI
  assistant session: TDD, no clock reads, audit log contract.
...
Launch your AI assistant from this directory. The orchestrator should read
`dispatcher.md` first and route every incoming question. The fifteen example questions
in that file describe the full coverage envelope.
